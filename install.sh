...
echo -e $COLOR_NONE
echo -e "$COLOR_WHITE[#1] Creating Virtual Environment"
virtualenv env

source env/bin/activate
echo -e $COLOR_NONE
echo -e "$COLOR_WHITE[#2] Virtual Environment created and activated"

echo -e $COLOR_NONE
echo -e "$COLOR_WHITE[#3] Installing dependencies..."
pip install -r requirements.txt

echo -e $COLOR_NONE
echo -e "$COLOR_WHITE[#4] Executing migrations, collecting static files"
python project/manage.py migrate
python project/manage.py collectstatic --noinput

echo -e $COLOR_NONE
echo -e "$COLOR_WHITE[#5] Creating superuser with default username: $ADMIN_USERNAME and email: $ADMIN_EMAIL"
